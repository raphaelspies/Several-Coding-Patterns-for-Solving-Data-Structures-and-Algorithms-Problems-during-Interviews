...
function removeElement(arr, key) {
  ...
  //base state: if "key" is not present, assign value at lesser pointer to the "next" valid value & increment
  // inverse: if "key" IS present, then i wil increment UNTIL it finds the next valid value
  // Then the element at nextElement will always be assigned to the next VALID value
  ...
  //by the time i finishes iterating, nextElement will have only incremented once for each VALID value
  return nextElement
}
